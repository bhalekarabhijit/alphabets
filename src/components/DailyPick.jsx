export default function DailyPick({ data, onAnalyzeStock, className = '' }) {
  ...
  return (
    <div className={`daily-pick-container fade-in ${className}`}>
      {/* Header */}
      <div className="daily-pick-header">
        <div className="daily-pick-badge">
          <span className="badge-icon">🎯</span>
          Today's AI Investment Pick
        </div>
        <div className="daily-pick-meta">
          <span>Analyzed {data.candidates_analyzed} stocks</span>
          <span className="meta-dot">·</span>
          <span>{new Date(data.timestamp).toLocaleDateString('en-IN', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</span>
        </div>
      </div>

      {/* Main Pick Card */}
      <div className="glass-card daily-pick-card">
        ...
        <div className="pick-confidence">
          ...
          <div className="confidence-bar-container">
            <div className={`confidence-bar ${confidenceClass}`} style={{ width: `${confidence}%` }}></div>
          </div>
        </div>
        ...
        <div className="pick-horizon">
          <span>⏱️ Time Horizon:</span>
          <strong>{pick.time_horizon}</strong>
          <span style={{ marginLeft: '16px' }}>📊 Position Size:</span>
        ...
        {/* Fundamental Thesis */}
        <div className="pick-section">
          <div className="section-subtitle">Fundamental Analysis</div>
          <p className="pick-thesis">{pick.fundamental_thesis}</p>
        </div>

        {/* Technical Thesis */}
        <div className="pick-section">
          <div className="section-subtitle">Technical Analysis</div>
        ...
        <div className="pick-section">
          ...
          <div className="pick-scores">
            {[
              ...
              { name: 'Risk (Safety)', value: scores.risk_score },
            ].map(s => (
        ...
        <div className="pick-actions">
          <button className="pick-analyze-btn" onClick={onAnalyzeStock}>
            📊 Deep Dive Analysis
          </button>
        </div>
      </div>
